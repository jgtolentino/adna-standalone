/**
 * Transaction Trends Export API
 *
 * Exports daily transaction trends data in CSV/XLSX/JSON format
...
export async function GET(request: NextRequest) {
  try {
      ...
      case 'csv': {
        ...
        return new NextResponse(csv, {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="trends_export_${new Date().toISOString().split('T')[0]}.csv"`,
          },
        });
      }
    ...
    console.error('[API /api/export/trends]', err);
    return NextResponse.json(
