/**
 * Geography Export API
 *
 * Exports regional performance data in CSV/XLSX/JSON format
...
export async function GET(request: NextRequest) {
  try {
      ...
      case 'csv': {
        ...
        return new NextResponse(csv, {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="geography_export_${new Date().toISOString().split('T')[0]}.csv"`,
          },
        });
      }
    ...
    console.error('[API /api/export/geography]', err);
    return NextResponse.json(
